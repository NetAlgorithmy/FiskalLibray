...
function OneBook(props) {const [data, setData] = useState(null);
  const handleWish = () => {
...
body: JSON.stringify({
  content_type: 9, // Replace with the primary key value of the ContentType object
  object_id: bookId,
  rating: rating
})
})
...
.catch(error => console.error(error));
  ...
  useEffect(() => {
    const bookId = window.location.pathname.split('/').pop();
    fetch(`http://127.0.0.1:8000/fiskal/article/${bookId}`)
